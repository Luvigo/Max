...
arduinoGenerator.finish = function(code) {
    ...
    if (setups && code.includes('void setup() {')) {
        // Insertar los setups justo después de la apertura del setup
...
arduinoGenerator.forBlock['arduino_if_elseif2_else'] = function(block) {
    const condition1 = arduinoGenerator.valueToCode(block, 'CONDITION', arduinoGenerator.ORDER_ATOMIC) || 'false';
    const statements1 = arduinoGenerator.statementToCode(block, 'DO');
    const condition2 = arduinoGenerator.valueToCode(block, 'CONDITION2', arduinoGenerator.ORDER_ATOMIC) || 'false';
    const statements2 = arduinoGenerator.statementToCode(block, 'DO2');
    const condition3 = arduinoGenerator.valueToCode(block, 'CONDITION3', arduinoGenerator.ORDER_ATOMIC) || 'false';
    const statements3 = arduinoGenerator.statementToCode(block, 'DO3');
    const elseStatements = arduinoGenerator.statementToCode(block, 'ELSE');
    return `  if (${condition1}) {\n${statements1}  } else if (${condition2}) {\n${statements2}  } else if (${condition3}) {\n${statements3}  } else {\n${elseStatements}  }\n`;
};
...
// Helper: Asegurar que las funciones de movimiento estén definidas
function ensureMovementFunctions() {
    // Si no se ha inicializado motores, agregar funciones con pines por defecto
    if (!arduinoGenerator.variables_['max_funciones']) {
        arduinoGenerator.includes_['servo'] = '#include <Servo.h>';
        
        if (!arduinoGenerator.variables_['max_servos']) {
            arduinoGenerator.variables_['max_servos'] = `Servo servoIzq;\nServo servoDer;`;
        }
        if (!arduinoGenerator.variables_['max_config']) {
            arduinoGenerator.variables_['max_config'] = `#define PIN_SERVO_IZQ 9\n#define PIN_SERVO_DER 10\n#define STOP_IZQ 90\n#define STOP_DER 90`;
        }
...
void detener() {
  ...
  servoDer.write(STOP_DER);
        ...
        // También necesitamos setup de los servos
        arduinoGenerator.setups_['max_servos'] = 'servoIzq.attach(PIN_SERVO_IZQ);\n  servoDer.attach(PIN_SERVO_DER);\n  detener();';
    }
}
...
// Helper: Asegurar que la función de distancia esté definida
function ensureDistanceFunction() {
    if (!arduinoGenerator.variables_['max_distancia_func']) {
        // Pines por defecto si no se inicializó
        if (!arduinoGenerator.variables_['max_distancia_pins']) {
            arduinoGenerator.variables_['max_distancia_pins'] = `#define PIN_TRIG 6\n#define PIN_ECHO 7`;
        }
        if (!arduinoGenerator.variables_['max_distancia_vars']) {
            arduinoGenerator.variables_['max_distancia_vars'] = `long _max_duracion;\nfloat _max_distancia;`;
        }
...
float medirDistancia() {
  ...
  return _max_duracion * 0.034 / 2;
        ...
        // Setup de pines
        arduinoGenerator.setups_['max_distancia'] = 'pinMode(PIN_TRIG, OUTPUT);\n  pinMode(PIN_ECHO, INPUT);';
    }
}
