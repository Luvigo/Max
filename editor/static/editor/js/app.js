...
async function uploadViaAgent(code, port, fqbn, onLog = () => {}) {
    ...
    const codeStr = (typeof code === 'string' ? code : String(code || '')).trim();
    if (!codeStr) {
        onLog('[UPLOAD] ✗ Compilación fallida: No hay código para compilar');
        return {
            success: false,
...
function getCodeForCompile() {
    ...
    // 1) Limpiar estado del generador (evita código vacío por estado desincronizado)
    try {
        arduinoGenerator.init(workspace);
    } catch (e) {
...
async function uploadCode() {
    ...
    try {
        // Re-obtener código justo antes de compilar (evita estado desincronizado tras awaits)
        const fresh = getCodeForCompile();
        if (fresh.error || !fresh.code || !fresh.code.trim()) {
            showToast(fresh.error || 'No hay código para subir', 'warning');
            logToConsole(`[UPLOAD] ${fresh.error || 'No hay código para compilar'}`, 'warning');
            btn.disabled = false;
            btn.setAttribute('aria-busy', 'false');
            btn.innerHTML = '<span>⬆️</span><span>Subir</span>';
            return;
        }
        const result = await uploadViaAgent(fresh.code, currentPort, currentBoard, (msg) => {
