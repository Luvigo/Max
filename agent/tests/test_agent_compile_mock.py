...
class TestAgentCompileMockOK(unittest.TestCase):
    ...
    def test_compile_ok_with_valid_code(self, mock_run):
        """Con código válido, compile mock OK (formato sketch.code)."""
    ...
    @patch('agent.agent.ARDUINO_CLI', '/usr/bin/arduino-cli')
    @patch('agent.agent.subprocess.run')
    def test_compile_ok_with_flat_code_format(self, mock_run):
        """Formato flat {code, fqbn} (igual que Verificar) debe funcionar para Upload."""
        def fake_run(cmd, *args, **kwargs):
            for i, c in enumerate(cmd):
                if c == '--output-dir' and i + 1 < len(cmd):
                    build_dir = Path(cmd[i + 1])
        ...
        self.assertEqual(resp.status_code, 200, f'Flat format debe funcionar: {resp.get_data(as_text=True)}')
        data = resp.get_json()
        self.assertTrue(data.get('ok'), f'Flat format ok debe ser true: {data}')
